...
server <- function(input, output, session) {
  ...
  observeEvent(input$file_in$datapath,
               {
                 sheets <- excel_sheets(path = input$file_in$datapath)
                 sheets_ok <- c()
                 sheets_notok <- c()
                 sheets_flag <- c()
                 # loop through all the sheets for four columns
                 ...
                 for (i in 1:length(sheets)) {
                   sheetContent = read_excel(path = input$file_in$datapath, sheet = sheets[i], .name_repair = ~make.unique(.x, sep = "_"))
                   musthave_cols <- unlist(lapply(must_have_columns, function(header) {
                     grep(paste0("^",header,"$"), names(sheetContent), ignore.case = TRUE)
                   }))
                   annotation_cols <- unlist(lapply(annotation_columns, function(header) {
                 ...
                 storage$sheets_flag = sheets_flag
                 ...
                 # limit the number of nodes here
                 storage$max_nodes_list = c(1,20,40,60,80,100,150,200)
                 storage$fullnet = fullnet
                 storage$plat_list = plat_list
                 storage$datalist = datalist
                 storage$sheets_ok = sheets_ok
                 
                 # storage$fullnet = list(
                 #   edges = all_edges,
                 #   nodes = all_nodes
                 # )
               })
  ...
  observe({
    updateSelectInput(session, "plat", choices = storage$plat_list, selected = storage$plat_list[1])
  })
  
  observe({
    updateSelectInput(session, "maxnodes", choices = storage$max_nodes_list, selected = storage$max_nodes_list[2])
  })
  ...
  output$SelectTrait = renderUI({
    ...
    if (input$plat == "ALL") {
      traitlist = c(storage$fullnet$nodes$id %>% sort())
    } else {
      ix = which(storage$fullnet$nodes$plat == input$plat)
      traitlist = c(storage$fullnet$nodes$id[ix] %>% sort())
    }
  ...
  output$network <- renderVisNetwork({
    ...
    subnet = neighbors(act_trait, storage$fullnet) %>% maxneighbors_noSTAT(storage$fullnet, limit = maxnodes) %>% nodes2network(storage$fullnet)
    # summary_network(subnet)
  ...
  output$tabs = renderUI({
    nTabs = length(storage$datalist)
    myTabs = lapply(seq_len(nTabs), function(i) {
      tabPanel(paste0(storage$sheets_ok[i]),width =32,
               #DT::dataTableOutput(paste0("datatable_",i))
               DT::dataTableOutput(paste0(storage$sheets_ok[i],"table"))
      )
    })
  ...
  observe({
  lapply(seq_len(length(storage$datalist)), function(i){
    output[[paste0(storage$sheets_ok[i],"table")]] <- DT::renderDataTable({
      data.frame(storage$datalist[i]
                 )}, filter = 'top', rownames = FALSE, selection = 'single',
      options = list(pageLength = 10, scrollX = TRUE))
  })
  })
  observe({
  lapply(seq_len(length(storage$sheets_ok)), function(tables){
    observeEvent(input[[paste0(storage$sheets_ok[tables],"table_rows_selected")]],{
      #print(input[[paste0(sheets_ok[tables],"table_rows_selected")]])
      #print(head(datalist[[tables]]))
      storage$asso_selected = storage$datalist[[tables]]$id[as.numeric(input[[paste0(storage$sheets_ok[tables],"table_rows_selected")]])]
...
ui <- dashboardPage(skin="red",
                    ...
                    dashboardSidebar( width = 150,
                                      sidebarMenu( id = "tabs",
                                                   menuItem("File upload", tabName = "Tab0", icon = icon("th")),
                                                   ...
                                                   menuItem("File formatting ...", tabName = "Tab7", icon = icon("th"))
                                      )
                    ),
                        ...
                        tabItem( tabName = "Tab0",
                                 fluidPage(
                                   fluidRow(
                                     column( width = 4,
                                             h3("Upload"),
                                             fileInput('file_in', 'Choose Excel File',accept = c(".xlsx")),
                                             tags$hr()
                                     ),
                                     tabBox( width = 5,
                                 ...
                                 fluidPage(
                                   ...
                                   fluidRow(
                                     column( width = 2,
                                             ...
                                             selectInput(inputId = "plat", label = "select platform", choices = NULL, selected = NULL)
                                     ),
                                     column( width = 6,
